...
import { Download, Eye, Link as LinkIcon, Plus, Save, Trash2, X } from "lucide-react";
import { useEffect, useState, type ReactNode } from "react";
import { saveResume } from "../api";
import type { ResumeData } from "../types";
import { emptyResume, formatResumeLinks, lines } from "../utils/resume";
      ...
      <div className="grid gap-6 2xl:grid-cols-[minmax(0,840px)_minmax(430px,1fr)] 2xl:items-start">
          ...
          <div className="border-b border-line bg-white px-5 py-4 sm:px-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-muted">Edit each section, preview the real resume, then export a clean PDF.</p>
              </div>
        ...
        <aside className="panel hidden h-fit overflow-hidden 2xl:sticky 2xl:top-5 2xl:block">
          ...
          <div className="max-h-[calc(100vh-150px)] overflow-auto bg-paper p-5">
            <ResumePreview resume={resume} scale="compact" />
          </div>
        </aside>
      </div>

      {previewOpen ? (
        <PreviewModal resume={resume} onClose={() => setPreviewOpen(false)} />
      ) : null}
    </>
  );
}
...
function PreviewModal({ resume, onClose }: { resume: ResumeData; onClose: () => void }) {
  return (
    <div
      ...
      >
        ...
        <div className="overflow-auto p-4 sm:p-8">
          <div className="mx-auto w-full max-w-[820px]">
            <ResumePreview resume={resume} scale="full" />
          </div>
        </div>
      </div>
    </div>
...
function ResumePreview({ resume, scale }: { resume: ResumeData; scale: "compact" | "full" }) {
  ...
  return (
    <article
      className={`mx-auto min-h-[720px] bg-white text-ink shadow-soft ${
        compact ? "p-7 text-[12px] leading-5" : "p-7 text-[13px] leading-6 sm:p-10"
      }`}
    >
      <header className="border-b border-ink pb-4">
        <h1 className={`${compact ? "text-2xl" : "text-3xl"} font-bold`}>{resume.fullName}</h1>
